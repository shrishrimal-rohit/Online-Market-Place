...
?>
<div <?php post_class( 'loop-product product style-02 ' . $column, $product->ID ); ?>>
	<div class="loop-product-inner">
		...
		if ( ( Insight::setting( 'shop_archive_quickview' ) == 1 ) && ! wp_is_mobile() ) {
			echo '<div class="quickview-btn hint--top hint--rounded hint--bounce hint--success" aria-label="' . esc_html__( 'Quick view', 'tm-organie' ) . '" data-pid="' . esc_attr( $id ) . '" data-pnonce="' . wp_create_nonce( 'woo_quickview' ) . '"><a class="quickview-icon" href="#"></a></div>';
		}
		woocommerce_template_loop_add_to_cart();
		if ( ( Insight::setting( 'shop_archive_compare' ) == 1 ) && class_exists( 'YITH_Woocompare' ) && ! wp_is_mobile() ) {
			echo '<div class="yith-compare-btn hint--top hint--rounded hint--bounce hint--success" aria-label="' . esc_html__( 'Compare', 'tm-organie' ) . '"><a href="/?action=yith-woocompare-add-product&amp;id=' . $id . '" class="compare" data-product_id="' . $id . '" rel="nofollow">' . esc_html__( 'Compare', 'tm-organie' ) . '</a></div>';
		}
		echo '</div>';
