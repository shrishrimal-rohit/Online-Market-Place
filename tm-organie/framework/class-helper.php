...
class Insight_Helper {
	...
	public static function get_woo_categories_dropdown( $dependency = '' ) {
		...
		if ( class_exists( 'WooCommerce' ) && class_exists( 'Insight_Vendor_Woocommerce' ) ) {
			$product_categories_dropdown = array();
			$first_value                 = array(
				'label' => esc_html__( 'Select category', 'tm-organie' ),
				'value' => '',
			);
			$vc_vendor_woo               = new Insight_Vendor_Woocommerce();
...
if ( class_exists( 'WooCommerce' ) && class_exists( 'Vc_Vendor_Woocommerce' ) ) {
	class Insight_Vendor_Woocommerce extends Vc_Vendor_Woocommerce {

		public function getCategoryChildsFull( $parent_id, $pos, $array, $level, &$dropdown ) {
			parent::getCategoryChildsFull( $parent_id, $pos, $array, $level, $dropdown );
		}
	}
}
